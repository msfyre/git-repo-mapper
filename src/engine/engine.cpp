...
void render_debug_information(SDL_Renderer *renderer, float runtime_dt, float render_dt)
{
    ...
    SDL_RenderDebugText(renderer, 5, 5, ((std::string) "DEBUG MODE: " + (Engine::IsDebug ? "TRUE" : "FALSE")).c_str());
    SDL_RenderDebugText(renderer, 5, 15, ((std::string) "FPS: " + fps_string).c_str());
}

void Engine::Run(const char *program_name, size_t max_fps)
{
    Engine::ProgramRuntime = new Runtime(max_fps);
    Engine::ProgramRenderer = new Renderer(Engine::ProgramRuntime, program_name, 640, 480);

    Engine::ProgramRenderer->SubscribeFunction([](SDL_Renderer *renderer, float runtime_dt, float render_dt) {
        if (Engine::IsDebug)
        {
